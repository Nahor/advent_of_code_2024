// Same as part2_combination, but split the `filter` into a `filter_map+filter`,
// which, somehow, is much faster
...
pub fn run(content: &[u8], min_save: usize) -> Result<u64> {
    ...
    let result: usize = path
        ...
        .enumerate()
        .tuple_combinations()
        .filter_map(|((start_i, start_pos), (end_i, end_pos))| {
            ...
            Some(normal_cost - cheat_cost)
        })
        .filter(|&saving| saving >= min_save)
        .count();
