/// Same as `part2_successors` but using `FxHashMap` instead of the standard `HashMap`
...
pub fn run(content: &[u8], blinks: usize) -> Result<u64> {
    ...
    let mut cache = FxHashMap::<u64, usize>::default();
    ...
    let (_, stones_final) = successors(Some((0, cache)), |(blink, cache)| {
        ...
        let mut next_cache = FxHashMap::<u64, usize>::default();
        cache
            .iter()
            .for_each(|(stone, count)| match stone.checked_ilog10().map(|d| d + 1) {
